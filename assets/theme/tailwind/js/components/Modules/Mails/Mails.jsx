	...
	return <div className="min-h-screen bg-white rounded-md shadow grid gap-4 xl:grid-cols-4 xl:gap-0 2xl:grid-cols-6">
		...
		<div className="border-t xl:border-t-0 xl:border-r xl:w-full 2xl:col-span-2">
			...
			<div className="max-h-screen overflow-y-auto py-2 flex flex-col">
				<ItemMail data={data} element={element} setElement={setElement}
...
function Destinators ({ prefix, data }) {
	return <div className="flex items-s gap-2">
		...
		<div className="flex flex-wrap gap-2">
			{data.map((dest, index) => {
				return <span className="bg-blue-50 text-sm py-1 px-2 rounded-md" key={index}>{dest.value}</span>
			})}
		</div>
	</div>
}

function Item ({ data, element, setElement, selection, selections, onSelect }) {
	return (data.length !== 0
		? data.map(elem => {
			return <div className={`cursor-pointer px-4 py-2 flex justify-between xl:flex-col gap-1 ${(selections.includes(elem.id) || (element && element.id === elem.id)) ? "bg-blue-50" : "hover:bg-gray-100"}`}
			...
			>
				...
				<div className="text-xs text-gray-600 xl:text-right">
					<div>{Sanitaze.toFormatDate(elem.createdAt, 'lll')}</div>
				</div>
			</div>
		})
		: null
